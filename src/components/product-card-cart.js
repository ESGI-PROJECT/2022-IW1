...
export class ProductCardCart extends Base {
  ...
  render() {
    return html`
      ...
      <a href="/product/${this.product.id}" class="card">
       ...
       <button @click="${this.removeProductFromCart}">Retirer du panier</button> 
      </div>
    `;
  }
  ...
  async editQuantity(event) {
    ...
    if(AppCart.state) {
      ...
      if(Math.sign(Number(event.target.value)) > 0) {
        const cart = await editProductQuantity(this.product.id,Number(event.target.value)) ;
